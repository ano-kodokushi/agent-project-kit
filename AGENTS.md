# AGENTS.md · 仓库治理书

> 本文件是 AI Agent 在本仓库工作的**唯一行为准则**。
> 每次会话开工前必须完整读取，再读 `docs/TASK_BRIEF.md` 与 `docs/PLAN.md`。

## 0. 项目速览

| 项 | 内容 |
|---|---|
| 项目名 | <!-- 填写 --> |
| 一句话目标 | <!-- 填写 --> |
| 技术栈 | <!-- 例：Node 22 + TypeScript + pnpm + PostgreSQL --> |
| 当前阶段 | <!-- 原型 / MVP / 迭代 / 维护 --> |

## 1. 开工前必读（按顺序）

1. 本文件 —— 规矩
2. `docs/TASK_BRIEF.md` —— 做什么、做到什么算完
3. `docs/PLAN.md` —— 按什么顺序做
4. `docs/BOARD.md` —— 当前进度与阻塞

**未读完之前禁止改动任何代码。**
三者冲突时优先级：本文件 > 任务书 > 计划书。

## 2. 目录结构约定

<!-- 替换为你的真实结构，Agent 会严格按此放置文件 -->

```
src/            源码
tests/          测试，与 src 目录结构镜像
docs/           文档（任务书、计划书、协调板）
scripts/        可执行脚本
```

- 新文件必须放对应目录，**禁止在根目录堆脚本**
- 目录名小写、中划线连接；组件文件用大驼峰，工具函数用小驼峰

## 3. 分支与提交

- 主分支 `main`，永远保持可运行
- 一个任务一个分支：`feat/T-012-用户登录`、`fix/T-015-空指针`、`chore/更新依赖`
- **禁止直接向 `main` 提交**
- 提交信息遵循 Conventional Commits：
  - `feat(scope): 简述` / `fix` / `docs` / `refactor` / `test` / `chore`
  - 正文写「为什么这么改」，不写「改了什么」（diff 里看得到）
- 一个任务一个提交（或少量原子提交），**不要把无关改动混进同一次提交**

### 怎么落地这条（不要靠自觉）

分工：**开分支交给 Agent（分支名有意义）**，**提交→推分支→合并 main 交给脚本**。

```powershell
# 1) Agent 先开分支（这一步不自动化，因为分支名承载任务语义）
& "C:\Program Files\Git\cmd\git.exe" switch -c feat/T-012-用户登录

# 2) 干完活，用脚本走完闭环：提交 → 推分支 → 快进合并 main → 推 main
& "C:\Program Files\nodejs\node.exe" push-task.mjs "<仓库绝对路径>" -m "feat(T-012): 用户登录"
```

`push-task.mjs` **会拒绝在 `main` 上直接提交**——那条红线由脚本执行，不靠提示词。
它还会：工作区无改动时拒绝产生空提交、非快进合并时中止并切回原分支（**不改写历史**）。
想留 PR 不自动合并，加 `--no-merge`；想先看它要做什么，加 `--dry-run`。

## 4. 代码风格

- 格式化与 lint 以配置文件为准：<!-- 例：eslint + prettier，禁止手改格式 -->
- 公开函数必须有简短注释说明用途与边界条件
- 禁止无意义的注释（`i++ // 自增`）
- 错误处理：明确区分「预期错误」与「程序 bug」，前者返回可处理的结果，后者快速失败
- 魔法数字与重复出现的字符串必须提取为常量

## 5. 依赖政策

- 新增运行时依赖前必须说明：为什么必须、包体积、许可协议、最近维护时间
- 能用标准库 / 现有依赖解决的不引新库
- **lock 文件必须一并提交**
- 禁止在代码中硬编码版本号以外的环境相关配置

## 6. 测试与验收

- 改行为必补测试；只改文档、注释、格式可不补
- 提交前本地必须全绿：`<!-- 例：pnpm test && pnpm lint -->`
- 计划书里每个任务自带验收标准，完成后**逐条自查并给出证据**，不要只说「已完成」

## 7. 绝对禁止（红线）

违反任一条，该次改动一律作废重来。

1. 禁止删除、跳过、`skip`、`xit`、注释掉测试来让构建变绿
2. 禁止硬编码密钥、Token、密码 —— 走环境变量，并在 `.env.example` 登记
3. 禁止大范围重构后声称「功能不变」却没跑测试
4. 禁止未经说明修改 CI 配置、依赖锁版本、类型检查严格度
5. 禁止对 `main` 执行 `push -f` / `reset --hard` / `clean -fd`
6. **禁止猜测需求** —— 任务书没写的先问，不要自行发挥加功能
7. 禁止一次改动超出当前任务范围的文件
8. 禁止把未验证的代码描述为「已完成」
9. 禁止为了让报错消失而放宽类型（`any`、`@ts-ignore`、关闭规则）

## 8. 完成定义（DoD）

一个任务算完成，必须同时满足：

- [ ] 计划书里该任务的验收标准逐条通过
- [ ] 测试全绿，lint 无新增警告
- [ ] 提交信息规范，分支已推送到远端
- [ ] `docs/BOARD.md` 状态与决策记录已更新
- [ ] 未引入任务书范围外的功能

## 9. 卡住时怎么办

按固定顺序处理，不要硬扛：

1. 同一问题重试超过 3 次仍失败 → 停止，写进 `BOARD.md` 的阻塞区，说明已尝试过什么
2. 发现任务书有漏洞或自相矛盾 → **不要自行补全**，提出具体疑问等确认
3. 发现范围外的既有 bug → 只记录，不修改
4. 上下文快满 → 先把当前结论写进 `BOARD.md`，再开新会话

## 10. 回报格式

每完成一个任务，按以下结构汇报：

```
任务：T-012 用户登录接口
改动文件：src/auth/login.ts, tests/auth/login.test.ts
验收结果：
  [x] 输入正确凭据返回 token —— 见 tests/auth/login.test.ts:23
  [x] 错误密码返回 401 —— 同上 :41
  [ ] 连续 5 次失败锁定账号 —— 未通过，原因：<说明>
遗留问题：<无 / 具体描述>
下一步建议：<建议>
```
